...
or to combine both steps, i.e. building and starting:
...
1. fork this repository and modify `docker-compose.yml`
2. clone your customized repository to a server of your choice

## 1. Fork this repo and customize it

Clone or fork `tei-publisher-docker-compose` to apply some modifications:

### 1.1. configure the docker image
...
In any case make sure to stop docker compose and rebuild before launching it again:
...
For security reasons we always hide eXist-db and TEI Publisher behind a proxy. The proxy redirects all incoming requests. By default, only a simple configuration is enabled: it runs on localhost and provides HTTP access only (port 80). We suggest to use this for testing the setup locally on a development machine.

Again the two most important configuration settings can be found in `docker-compose.yml`:
...
Once you have root access to your server, ssh into it and clone your customized docker compose configuration repository. Assuming that you have a domain name for the server, edit `docker-compose.yml` and set `SERVER_NAME`:
...
Launch the docker compose again. The server should start responding to port 80 (i.e. HTTP-only). To enable HTTPS one additional step is required:

### Acquire an SSL certificate

1. Make sure you have started the services
...
3. Rename `nginx/templates/default.ssl.conf.off` to `nginx/templates/default.ssl.conf.template`, thus enabling this configuration file
...
1. register a cron job to call this script once a day. Call `crontab -e` and add a line:
   ```
